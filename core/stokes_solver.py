...
from scipy.sparse.linalg import spsolve, lsmr, cg, gmres, splu, spilu, LinearOperator
import time
import warnings
...
class StokesConfig:
    ...
    strain_rate_dependent_viscosity: bool = False
...
class StokesMesh:
    ...
    def from_unstructured_tetrahedra(cls, nodes: np.ndarray,
                                     elements: np.ndarray):
        """Build a 3D Stokes mesh from positively oriented tetrahedra."""
        mesh = cls.__new__(cls)
        mesh.nodes = np.asarray(nodes, dtype=float)
        mesh.elements = []
        for element in np.asarray(elements, dtype=int):
            tetra = element.tolist()
            coords = mesh.nodes[tetra]
            determinant = np.linalg.det((coords[1:] - coords[0]).T)
            if determinant < 0.0:
                tetra[2], tetra[3] = tetra[3], tetra[2]
                determinant = -determinant
            if determinant <= 1e-12:
                raise ValueError("unstructured mesh contains a degenerate tetrahedron")
            mesh.elements.append(tetra)
        ...
        mesh.lx, mesh.ly, mesh.lz = extents.tolist()
        mesh.dim = 3
        mesh.n_dofs_per_node = 5
        ...
        for k in range(nz):
            for j in range(ny):
                for i in range(nx):
                    ...
                    for tet in tets:
                        coords = self.nodes[tet]
                        if np.linalg.det((coords[1:] - coords[0]).T) < 0.0:
...
class GlobalStokesAssembler:
    ...
    def assemble(self, viscosity: np.ndarray,
        ...
        for elem_nodes in self.mesh.elements:
            ...
            gravity = np.zeros(self.mesh.dim)
            gravity[-1] = -1.0
    ...
    def assemble_rhs(self, temperature: np.ndarray,
        ...
        """Assemble buoyancy forcing for the coupled Stokes system."""
...
class StokesBoundaryConditions:
    ...
    def __init__(self, mesh: StokesMesh, config: StokesConfig):
        self.mesh = mesh
        self.config = config
        self.nodal_dofs = mesh.n_dofs_per_node
    ...
    def apply(self, A: sp.spmatrix, b: np.ndarray) -> Tuple[sp.spmatrix, np.ndarray]:
        ...
        temperature_offset = self.mesh.dim + 1
        for node_id in self.mesh.bottom_nodes:
            dof = node_id * self.nodal_dofs + temperature_offset
        ...
        if self.config.boundary_velocity == 'free_slip':
            ...
            for node_id in self.mesh.bottom_nodes + self.mesh.top_nodes:
                dof_vertical = node_id * self.nodal_dofs + self.mesh.dim - 1
                self._set_dirichlet(A, b, dof_vertical, 0.0)
            ...
            if self.mesh.dim == 3:
                for node_id in self.mesh.front_nodes + self.mesh.back_nodes:
        ...
        p_dof = bottom_center * self.nodal_dofs + self.mesh.dim
...
class ViscosityModel:
    ...
    def compute(self, temperature: np.ndarray,
        ...
        # 应变率依赖 (幂律)
        if self.config.strain_rate_dependent_viscosity and strain_rate is not None:
            n = 3.5  # 橄榄石应力指数
...
class TemperatureAdvection:
    ...
    def compute_strain_rate(self, velocity: np.ndarray) -> np.ndarray:
        ...
        if self.mesh.dim == 3:
            ...
            from finite_elements.quadrature import tetra_points_weights
            basis = LagrangeTetra(1)
            quad_pts, quad_wts = tetra_points_weights(1)
        else:
            basis = LagrangeTriangle(1)
        ...
        for elem_nodes in self.mesh.elements:
            ...
            for q, w in zip(quad_pts, quad_wts):
                ...
                velocity_gradient = np.zeros((self.mesh.dim, self.mesh.dim))
                for a in range(len(elem_nodes)):
                    n_idx = elem_nodes[a]
                    node_velocity = np.array([
                        velocity[n_idx * self.mesh.n_dofs_per_node + component]
                        for component in range(self.mesh.dim)
                    ])
                    velocity_gradient += np.outer(node_velocity, dN_dx_[a])
                strain = 0.5 * (velocity_gradient + velocity_gradient.T)
                eps_ii = np.sqrt(0.5 * np.sum(strain ** 2))
                for a in range(len(elem_nodes)):
    ...
    def advect(self, temperature: np.ndarray, velocity: np.ndarray,
        ...
        for elem_nodes in self.mesh.elements:
            ...
            # 提取节点速度和温度
            u_elem = np.array([
                [velocity[n * self.mesh.n_dofs_per_node + component]
                 for component in range(self.mesh.dim)]
                for n in elem_nodes])
            T_elem = temperature[elem_nodes]
...
class _PyAMGVelocitySolver:
    """PyAMG hierarchy adapter for the block solver's V-cycle interface."""
    ...
    def setup(self, matrix, rhs=None):
        ...
        self.multilevel = pyamg.smoothed_aggregation_solver(
            matrix.tocsr(), symmetry='symmetric', strength='symmetric')
        self.is_setup = True
    ...
    def update_matrix(self, matrix):
        ...
        self.multilevel.levels[0].A = matrix.tocsr()
        smoothing.rebuild_smoother(self.multilevel.levels[0])
        for index in range(len(self.multilevel.levels) - 1):
            fine = self.multilevel.levels[index]
            coarse = self.multilevel.levels[index + 1]
            coarse.A = (fine.R @ fine.A @ fine.P).tocsr()
            if index + 1 < len(self.multilevel.levels) - 1:
                smoothing.rebuild_smoother(coarse)

    def v_cycle(self, level, rhs, initial):
        return self.multilevel.solve(
            np.asarray(rhs), x0=np.asarray(initial), tol=0.0,
...
class BlockStokesSolver:
    ...
    def __init__(self, mesh: StokesMesh, config: StokesConfig):
        ...
        self._picard_step = 0
        self._reused_velocity_solver = None
        ...
        self.reused_hierarchy_updates = 0
        self.reused_hierarchy_update_time = 0.0
        self.approximate_schur_solves = 0
        self.schur_velocity_ilu_setups = 0
        self.schur_velocity_ilu_setup_time = 0.0
        self.schur_velocity_ilu_fallbacks = 0
        self.schur_velocity_lu_setups = 0
        self.schur_velocity_lu_setup_time = 0.0
        self.schur_velocity_lu_fallbacks = 0
    ...
    def get_stats(self) -> Dict:
        return {
            ...
            'reused_hierarchy_updates': self.reused_hierarchy_updates,
            'reused_hierarchy_update_time': self.reused_hierarchy_update_time,
            'approximate_schur_solves': self.approximate_schur_solves,
            'schur_velocity_ilu_setups': self.schur_velocity_ilu_setups,
            'schur_velocity_ilu_setup_time': self.schur_velocity_ilu_setup_time,
            'schur_velocity_ilu_fallbacks': self.schur_velocity_ilu_fallbacks,
            'schur_velocity_lu_setups': self.schur_velocity_lu_setups,
            'schur_velocity_lu_setup_time': self.schur_velocity_lu_setup_time,
            'schur_velocity_lu_fallbacks': self.schur_velocity_lu_fallbacks,
    ...
    def _new_velocity_solver(self, A_vel):
        if self.config.velocity_amg_backend == 'pyamg':
        ...
        try:
            ...
            try:
                x, info = gmres(
                    ...
                    maxiter=max(50, min(self.config.velocity_solver_max_iterations,
                                        2 * A_vel.shape[0])),
    ...
    def extract_blocks(self, A_full: sp.spmatrix, b_full: np.ndarray,
        ...
        """从全耦合系统提取速度块矩阵和右端项"""
        n_nodes = self.mesh.n_nodes
        nodal_dofs = self.mesh.n_dofs_per_node

        # 速度块: 只取 ux, uy DOF
        vel_indices = [
            node * nodal_dofs + component
            for node in range(n_nodes) for component in range(self.mesh.dim)
        ]
        ...
        # 散度算子 B: 压力DOF × 速度DOF
        p_indices = [i * nodal_dofs + self.mesh.dim for i in range(n_nodes)]
    ...
    def solve_with_velocity_block(self, A_full, b_full, A_vel_prebuilt,
        ...
        p_idx = [i * nodal_dofs + self.mesh.dim for i in range(n_nodes)]
        t_idx = [i * nodal_dofs + self.mesh.dim + 1 for i in range(n_nodes)]
        ...
        schur_velocity_ilu = {'factor': None, 'failed': False}
        schur_velocity_lu = {'factor': None, 'failed': False}

        def apply_schur_velocity_inverse(rhs_u):
            if self.config.schur_velocity_inverse == 'vcycle':
                if not getattr(vel_solver, 'is_setup', False):
                    setup_start = time.time()
                    vel_solver.setup(A_vel, rhs_u)
                    setup_time = time.time() - setup_start
                    if getattr(vel_solver, '_hierarchy_source', 'traditional') == 'adapted':
                        self.adapted_hierarchy_setups += 1
                        self.adapted_hierarchy_setup_time += setup_time
                    else:
                        self.traditional_hierarchy_setups += 1
            ...
            if self.config.schur_velocity_inverse == 'ilu':
                if schur_velocity_ilu['factor'] is None and not schur_velocity_ilu['failed']:
                    try:
                        t0 = time.time()
                        schur_velocity_ilu['factor'] = spilu(
                            A_vel.tocsc(), drop_tol=1e-4, fill_factor=10)
                        self.schur_velocity_ilu_setups += 1
                        self.schur_velocity_ilu_setup_time += time.time() - t0
                    except Exception:
                        self.schur_velocity_ilu_fallbacks += 1
                        schur_velocity_ilu['failed'] = True
                if schur_velocity_ilu['factor'] is not None:
                    return schur_velocity_ilu['factor'].solve(rhs_u)
            if self.config.schur_velocity_inverse == 'lu':
                if schur_velocity_lu['factor'] is None and not schur_velocity_lu['failed']:
                    try:
                        t0 = time.time()
                        schur_velocity_lu['factor'] = splu(A_vel.tocsc())
                        self.schur_velocity_lu_setups += 1
                        self.schur_velocity_lu_setup_time += time.time() - t0
                    except Exception:
                        self.schur_velocity_lu_fallbacks += 1
                        schur_velocity_lu['failed'] = True
                if schur_velocity_lu['factor'] is not None:
                    return schur_velocity_lu['factor'].solve(rhs_u)
            return solve_velocity(rhs_u, phase='schur_columns')
        ...
        def schur_matvec(p_vec):
            return pressure_block @ p_vec - B @ apply_schur_velocity_inverse(G @ p_vec)
        ...
        try:
            if self.config.pressure_solver == 'approx_schur':
                p = spsolve(approximate_schur, rhs_p)
                self.approximate_schur_solves += 1
                if not np.isfinite(p).all():
                    raise ValueError("non-finite approximate Schur solve")
            else:
...
class PicardStokesSolver:
    ...
    def __init__(self, config: StokesConfig = None, mesh: StokesMesh = None):
        self.config = config or StokesConfig()
        self.mesh = mesh or StokesMesh(
            nx=self.config.nx, ny=self.config.ny, nz=self.config.nz,
            lx=self.config.lx, ly=self.config.ly, lz=self.config.lz,
        )
    ...
    def _setup_meta_amg(self):
        ...
        try:
            ...
            n_nodes = self.mesh.n_nodes
            target_max_matrix_size = max(200, n_nodes * 2)
            if self.config.meta_training_max_matrix_size is not None:
                target_max_matrix_size = min(
                    target_max_matrix_size,
                    self.config.meta_training_max_matrix_size)
            mcfg = MetaAMGConfig(
                min_matrix_size=max(64, n_nodes // 4),
                max_matrix_size=max(64, target_max_matrix_size),
    ...
    def initialize_temperature(self):
        """初始化温度场: 线性分布 + 小扰动"""
        vertical = self.mesh.nodes[:, -1]
        vertical_length = self.config.lz if self.mesh.dim == 3 else self.config.ly
        T_linear = (self.config.boundary_temp_bottom +
                    (self.config.boundary_temp_top -
                     self.config.boundary_temp_bottom) * vertical / vertical_length)

        # 随机扰动触发对流
        perturbation = 0.01 * np.random.randn(self.mesh.n_nodes)
        perturbation *= np.sin(np.pi * vertical / vertical_length)
    ...
    def collect_picard_sequence(self, max_iterations: int = None,
        ...
        vel_indices = np.array(
            [node * self.mesh.n_dofs_per_node + component
             for node in range(self.mesh.n_nodes) for component in range(self.mesh.dim)],
            dtype=np.int64,
        )
        records = []
    ...
    def _compute_nusselt(self) -> float:
        ...
        if self.mesh.dim == 3:
            ...
            vertical_spacing = self.config.ly / self.config.ny
            row_stride = self.config.nx + 1
        for top_node in self.mesh.top_nodes:
            below_node = top_node - row_stride
            if below_node < 0:
                continue
            dT_vertical = ((self.temperature[top_node] - self.temperature[below_node]) /
                           vertical_spacing)
            top_fluxes.append(-dT_vertical / delta_T)
